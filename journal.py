from typing import List, Optional, TypeVar, Generic, Union, Literal
...
class PublicationFrequency(BaseModel):
    ...
    frequency: Optional[Sourced[str]] = Field(default=None, description="Human readable frequency label (e.g., 'Monthly', 'Quarterly').")
    issues_per_year: Optional[Sourced[int]] = Field(default=None, description="Exact number of issues per year when available.")
...
class ReviewProcess(BaseModel):
    ...
    description: Optional[Sourced[str]] = Field(default=None, description="Freeform description of the review workflow.")

class Membership(BaseModel):
    """Information about society or institutional membership models."""
    type: Optional[Sourced[str]] = Field(default=None, description="Membership type (e.g., 'society', 'institutional').")
...
class Fee(BaseModel):
    ...
    currency: Optional[Sourced[SupportedCurrency]] = Field(default=None, description="ISO 4217 currency code (USD or EUR).")
...
class APC(BaseModel):
    ...
    category: Optional[Sourced[str]] = Field(default=None, description="Optional category label (e.g., Frontiers-style categories).")
...
class Editor(BaseModel):
    ...
    affiliations: Optional[List[Sourced[str]]] = Field(default_factory=list, description="List of institutional affiliations.")
...
class AdditionalInformation(BaseModel):
    ...
    quality_assurance: Optional[Sourced[str]] = Field(default=None, description="Notes on quality assurance or peer review standards.")
    other: Optional[List[Sourced[str]]] = Field(default_factory=list, description="Other minor publisher-specific fields.")
...
class JournalScope(BaseModel):
    ...
    journal_sections: Optional[List[Sourced[str]]] = Field(default_factory=list, description="Sections within the journal.")
...
class SubmissionInfo(BaseModel):
    ...
    article_types: Optional[List[ArticleType]] = Field(default_factory=list, description="List of supported article types.")
...
class ReviewAndPolicy(BaseModel):
    ...
    additional_information: Optional[AdditionalInformation] = Field(default=None, description="Publisher-specific policy details.")

class Pricing(BaseModel):
    """Article processing charges and discounts."""
    article_processing_charges: Optional[List[APC]] = Field(default_factory=list, description="Article Processing Charges.")
